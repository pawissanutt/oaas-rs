...
== Introduction
A Rust reimplementation of https://github.com/hpcclab/OaaS[Oparaca] — an Object-as-a-Service platform with separate control and data planes. Control plane manages class/function deployments via Kubernetes; data plane handles invocations, routing, and stateful object storage.
...
Install the CLI tool to interact with the platform:
...
----
# Set context
oprc-cli context set --pm-url http://localhost:8080

# Invoke a function
oprc-cli fn invoke --cls-id example --fn-id echo --payload "hello"

# Manage objects
oprc-cli object set --cls-id users 0 user-42 -s name=alice
----

See link:tools/oprc-cli/README.md[CLI Documentation] for full reference.
...
graph TB
    subgraph CP["Control Plane"]
        PM["📦 PM<br/>Package Manager<br/>REST API"]
        CRM["⚙️ CRM<br/>Class Runtime Manager<br/>K8s Controller"]
    end
    
    subgraph DP["Data Plane"]
        GW["🚪 Gateway<br/>REST/gRPC Ingress"]
        RT["🔀 Router<br/>Zenoh Messaging"]
        ODGM["🗄️ ODGM<br/>Object Data Grid"]
        FN["🔧 Function<br/>Runtimes"]
    end
...
|Stateless ingress, REST/gRPC → Zenoh translation
...
=== Features
* **String Object IDs**: Human-readable addressing and richer per-entry semantics. See link:docs/ODGM_FEATURES.md[ODGM Features] for details.
* **Granular Storage**: Per-entry persistence for efficient updates.
* **Object Events**: Reactive pipeline with per-entry triggers.
...
=== Typical Flow
1. **Deploy**: Client → PM (REST) → CRM (gRPC) → upsert ClassRuntime CRD → reconcile → apply function + ODGM resources
2. **Invoke**: Client → Gateway (REST/gRPC) → Router (Zenoh) → Function runtime or ODGM
3. **Status**: Client → PM → CRM → read ClassRuntime conditions
4. **Delete**: Client → PM → CRM → mark CRD for deletion → cleanup resources
...
[source,bash]
----
# Run development environment (ODGM, etc.)
docker compose -f docker-compose.dev.yml up

# Run system E2E tests (requires Kind)
just system-e2e
----
...
├── oprc-config/     # Configuration utilities
├── oprc-cp-storage/ # Control plane storage (memory, etcd)
├── oprc-dp-storage/ # Data plane storage (memory, redb, fjall, rocksdb)
├── oprc-grpc/       # Protobuf definitions & clients
├── oprc-invoke/     # Invocation models
├── oprc-models/     # Domain models
├── oprc-observability/ # Metrics and tracing
├── oprc-test-utils/ # Testing utilities
├── oprc-zenoh/      # Zenoh config helpers
└── oprc-zrpc/       # Zenoh RPC implementation

control-plane/
├── oprc-pm/         # Package Manager (REST API)
└── oprc-crm/        # Class Runtime Manager (K8s controller)

data-plane/
├── oprc-gateway/    # REST/gRPC ingress
├── oprc-router/     # Zenoh routing
