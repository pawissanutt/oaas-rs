...
use oprc_cp_storage::traits::{StorageFactory, PackageStorage};
use oprc_models::{
    package::{FunctionBinding, OClass, OFunction, OPackage, PackageMetadata, StateSpecification},
    enums::FunctionType,
};
use std::sync::Arc;
...
fn sample_package(name: &str) -> OPackage {
    OPackage {
        ...
        disabled: false,
        metadata: PackageMetadata { author: Some("test".into()), description: None, tags: vec![], created_at: None, updated_at: None },
    classes: vec![OClass { key: "klass".into(), description: None, state_spec: Some(StateSpecification::default()), function_bindings: vec![FunctionBinding { name: "f1".into(), function_key: "f1".into(), ..Default::default() }], disabled: false }],
    functions: vec![OFunction { key: "f1".into(), function_type: FunctionType::Custom, description: None, provision_config: None, config: Default::default() }],
        dependencies: vec![],
...
async fn pm_can_bootstrap_with_etcd_storage_and_do_crud() -> Result<()> {
    ...
    let image = GenericImage::new("bitnami/etcd", "3.5")
        ...
        .with_wait_for(WaitFor::message_on_either_std("grpc service status changed"))
    ...
    unsafe {
        ...
        std::env::set_var("OPRC_PM_ETCD_ENDPOINTS", &format!("127.0.0.1:{}", host_port));
    ...
    let fetched = package_service.get_package("pkg-pm").await?.expect("present");
