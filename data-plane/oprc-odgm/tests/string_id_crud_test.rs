...
#[tokio::test(flavor="multi_thread")] async fn string_object_create_get_roundtrip() {
    ...
    let req = SetObjectRequest { cls_id: "coll".into(), partition_id: 0, object_id: 0, object: Some(obj), object_id_str: Some("order-abc-1".into()) };
    client.set(req).await.expect("set string object");

    // Get object via string ID
    let get_req = SingleObjectRequest { cls_id: "coll".into(), partition_id: 0, object_id: 0, object_id_str: Some("order-abc-1".into()) };
    let resp = client.get(get_req).await.expect("get string object");
    assert!(resp.into_inner().obj.is_some(), "object missing");
}

#[tokio::test(flavor="multi_thread")] async fn both_ids_rejected() {
    let cfg = TestConfig::new().await;
    let env = TestEnvironment::new(cfg.clone()).await;
    env.start_odgm().await.expect("start odgm");
    env.create_test_collection("coll").await.expect("create collection");
    ...
    let obj = ObjData { ..Default::default() };
    let req = SetObjectRequest { cls_id: "coll".into(), partition_id: 0, object_id: 42, object: Some(obj), object_id_str: Some("order-x".into()) };
