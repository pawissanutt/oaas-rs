...
fn metadata() -> ShardMetadata { ShardMetadata { id: 9101, collection: "v2act".into(), partition_id: 0, owner: None, primary: None, replica: vec![], replica_owner: vec![], shard_type: "memory".into(), options: Default::default(), invocations: Default::default(), storage_config: None, replication_config: None, consistency_config: None } }
...
async fn v2_create_update_delete_actions() {
    ...
    for _ in 0..3 { let evt = tokio::time::timeout(std::time::Duration::from_secs(2), rx.recv()).await.expect("timeout").expect("evt"); actions.push(evt.ctx.changed[0].action); }
    assert_eq!(actions, vec![MutAction::Create, MutAction::Update, MutAction::Delete]);

    // Idempotent delete (should not produce another event)
    shard.delete_entry(oid, "k").await.unwrap();
    let maybe_extra = tokio::time::timeout(std::time::Duration::from_millis(200), rx.recv()).await;
    assert!(maybe_extra.is_err(), "idempotent delete produced unexpected event");
}
