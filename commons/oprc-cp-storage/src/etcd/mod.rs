...
#[cfg(feature = "etcd")]
use etcd_rs::{Client, ClientConfig, GetOptions, PutOptions, DeleteOptions};
#[cfg(feature = "etcd")]
...
impl EtcdStorageFactory {
    ...
    async fn create_client(&self) -> Result<Client, StorageError> {
        ...
        if let Some(ref creds) = self.credentials {
            config = config.with_auth(creds.username.clone(), creds.password.clone());
        }
...
#[cfg(feature = "etcd")]
impl EtcdStorage {
    pub async fn new(factory: &EtcdStorageFactory) -> Result<Self, StorageError> {
...
impl PackageStorage for EtcdStorage {
    ...
    async fn get_package(&self, name: &str) -> StorageResult<Option<OPackage>> {
        ...
        match self.client.get(key, None).await {
            ...
            Err(etcd_rs::Error::GRpcStatus(status)) if status.code() == tonic::Code::NotFound => {
                Ok(None)
            }
    ...
    async fn list_packages(&self, _filter: PackageFilter) -> StorageResult<Vec<OPackage>> {
