...
use oprc_grpc::{CapabilitiesRequest, data_service_client::DataServiceClient};

pub async fn handle_capabilities_command(
    conn: &ConnectionArgs,
    json: bool,
) -> anyhow::Result<()> {
    let url = conn.grpc_url.clone().ok_or_else(|| {
        anyhow::anyhow!(
            "gRPC URL required (set context gateway_url or pass --grpc-url)"
        )
    })?;
    ...
    if json {
        ...
        println!(
            ...
            resp.string_ids,
            resp.string_entry_keys,
            resp.granular_entry_storage
        );
    }
    Ok(())
}
