...
=== Event System (V2 Pipeline)

Pure asynchronous trigger execution for reactive applications using the V2 per-entry event pipeline. Events are automatically triggered by entry-level operations and dispatched via Zenoh's fire-and-forget mechanism.

NOTE: V2 is the sole event pipeline. Bridge (object-level) events have been removed.
...
+
Per-entry data change events (create/update/delete) are automatically detected during:
+
- Entry-level storage operations (`batch-set`, `set-value`)
- Granular field modifications with metadata tracking
- Entry lifecycle changes (creation, updates, tombstone deletion)
+
...
Events are configured via the `ObjectEvent` field in <<ObjData,ObjData>>. The V2 pipeline supports both numeric and string-keyed entry triggers.

* *Data Events*: Triggered on per-entry changes
+
[source,yaml]
----
# Example: Numeric key triggers notification on update
data_trigger:
  42:
    on_update:
...
# Example: String key triggers indexing on create
data_trigger_str:
  "metadata.tags":
    on_create:
      - cls_id: "search_service"
        partition_id: 1
        fn_id: "index_tags"
    on_update:
      - cls_id: "search_service"
...
==== Performance Characteristics (V2 Pipeline)

Measured via Criterion benchmarks (`cargo bench --bench bench_event`):

* *Event Context Creation*: 25.7 ns (numeric keys), 34.3 ns (string keys)
* *Event Collection Triggering*: 45.6 ns
* *Payload Serialization*: 76 ns (Protobuf), 334 ns (JSON) - Protobuf 4.4x faster
...
All operations complete in sub-microsecond time, making V2 suitable for high-throughput reactive applications.
