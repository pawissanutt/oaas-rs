...
impl StorageFactory {
    ...
    pub async fn create_memory() -> StorageResult<crate::backends::memory::MemoryStorage> {
        let config = StorageConfig::memory();
        crate::backends::memory::MemoryStorage::new(config).await
    }
    
    /// Create a temporary storage backend for testing
    pub async fn create_temp() -> StorageResult<crate::backends::memory::MemoryStorage> {
...
impl StorageBuilder {
    ...
    pub async fn build(self) -> StorageResult<crate::backends::memory::MemoryStorage> {
        // For now, only support memory storage in the PoC
        match self.config.backend_type {
            StorageBackendType::Memory => {
                crate::backends::memory::MemoryStorage::new(self.config).await
            }
            _ => Err(StorageError::configuration("Only memory backend supported in PoC")),
        }
    }
}
...
mod tests {
    ...
    fn test_available_backends() {
        ...
        assert!(StorageFactory::is_backend_available(&StorageBackendType::Memory));
    }
    ...
    fn test_storage_builder() {
        ...
        assert_eq!(builder.config.properties.get("test_key"), Some(&"test_value".to_string()));
    }
