...
pub struct Opts {
    ...
    pub loop_count: u32,
    /// Report file to save the result.
    #[arg(long)]
...
impl InvocationBench {
    pub async fn new(sessions: Vec<zenoh::Session>, opts: Opts) -> Self {
            ...
            Ok(resp) => {
                if resp.status == ResponseStatus::Okay as i32 {
                    ...
                    let env_id: u32 = resp
                        ...
                        .unwrap_or(0);
                    let status = rlt::Status::success(200 + env_id as i64);
...
fn main() {
    ...
    let _ = rt.block_on(async {
        ...
        for i in 0..session_count {
            let s = zenoh::open(oprc_zenoh.create_zenoh())
                .await
        ...
        for _ in 0..opts.loop_count {
            let bench =
                InvocationBench::new(sessions.clone(), opts.clone()).await;
            let report = rlt::cli::run(opts.bench_opts, bench)
                .await
                .expect("Failed to run benchmark");
            reports.push(report);
        }
        if let Some(report_path) = &opts.report_file {
            ...
            for report in reports {
                let r_serde = rlt::reporter::build_serde_report(&report);
                let r_str = serde_json::to_vec(&r_serde)
