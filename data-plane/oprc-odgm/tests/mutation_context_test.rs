...
use oprc_odgm::shard::unified::{ObjectUnifiedShard, ShardError, UnifiedShardFactory};

type TestShard = ObjectUnifiedShard<AnyStorage, NoReplication<AnyStorage>, oprc_odgm::events::EventManagerImpl<AnyStorage>>;

fn metadata() -> ShardMetadata { ShardMetadata { id: 8601, collection: "mutation".into(), partition_id: 0, owner: None, primary: None, replica: vec![], replica_owner: vec![], shard_type: "memory".into(), options: Default::default(), invocations: Default::default(), storage_config: None, replication_config: None, consistency_config: None } }
...
async fn make_shard() -> Result<TestShard, ShardError> {
    ...
    let cfg = UnifiedShardConfig { enable_string_ids: true, max_string_id_len: 160, granular_prefetch_limit: 256 };
...
async fn test_mutation_context_create_update_counts() -> Result<(), ShardError> {
    ...
    shard.batch_set_entries(oid, vec![("k2".to_string(), val("v3")), ("k3".to_string(), val("v4"))].into_iter().collect(), None).await?; // creates
    ...
    assert_eq!(meta.object_version, 3, "expected object_version 3 after create, update, batch");
    Ok(())
}