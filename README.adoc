...
=== String Object IDs & String Entry Keys (Preview → GA)
...
* Entries:
** Numeric entry keys remain `u32` → `ValData`
...
* CLI Usage Examples:
** Create string-id object: `oprc-cli object set --cls-id users 0 user-42 -s name=alice -s role=admin`
** Get single string field: `oprc-cli object set --cls-id users 0 user-42 --key name`
...
The project includes a comprehensive end-to-end test suite that validates the entire platform on a local Kubernetes cluster using Kind. See link:tests/system_e2e/README.md[tests/system_e2e/README.md] for details.
