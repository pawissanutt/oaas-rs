...
pub fn parse_id(id_str: &str) -> Result<(u16, u64), ParseIdError> {
    ...
    let partition_id_bytes =
        decode(Alphabet::Rfc4648 { padding: false }, parts[0])
            .ok_or(ParseIdError::PartitionIdDecodeError)?;
    let object_id_bytes =
        decode(Alphabet::Rfc4648 { padding: false }, parts[1])
    ...
    let partition_id =
        u16::from_be_bytes(partition_id_bytes[..2].try_into().unwrap());
    let object_id =
        u64::from_be_bytes(object_id_bytes[..8].try_into().unwrap());
...
fn to_string(partition_id: u16, object_id: u64) -> String {
    ...
    let partition_id_str = base32::encode(
        Alphabet::Rfc4648 { padding: false },
        &partition_id_bytes,
    );
    let object_id_str =
        base32::encode(Alphabet::Rfc4648 { padding: false }, &object_id_bytes);
