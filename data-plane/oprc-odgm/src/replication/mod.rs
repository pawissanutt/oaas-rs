...
pub trait ReplicationLayer: Send + Sync + Clone {
    ...
    async fn replicate_read(&self, request: ShardRequest) -> Result<ReplicationResponse, Self::Error>;
...
impl ReplicationLayer for NoReplication {
    ...
    async fn replicate_write(&self, _request: ShardRequest) -> Result<ReplicationResponse, Self::Error> {
    ...
    async fn replicate_read(&self, _request: ShardRequest) -> Result<ReplicationResponse, Self::Error> {
        Ok(ReplicationResponse {
            status: ResponseStatus::Applied,
            data: None,
            metadata: HashMap::new(),
