...
async fn package_crud_endpoints() -> Result<()> {
    ...
    let create_request = Request::builder().method("POST").uri("/api/v1/packages").header("content-type", "application/json").body(Body::from(serde_json::to_string(&test_package)?))?;
    let response = app.clone().oneshot(create_request).await?;
    assert_eq!(response.status(), StatusCode::CREATED);
    let list_request = Request::builder().uri("/api/v1/packages").body(Body::empty())?;
    ...
    let get_request = Request::builder().uri(&format!("/api/v1/packages/{}", test_package.name)).body(Body::empty())?;
    let response = app.clone().oneshot(get_request).await?; assert_eq!(response.status(), StatusCode::OK);
    let body = axum::body::to_bytes(response.into_body(), usize::MAX).await?;
    let retrieved_package: OPackage = serde_json::from_slice(&body)?; assert_eq!(retrieved_package.name, test_package.name);
    let delete_request = Request::builder().method("DELETE").uri(&format!("/api/v1/packages/{}", test_package.name)).body(Body::empty())?;
    let response = app.clone().oneshot(delete_request).await?; assert_eq!(response.status(), StatusCode::NO_CONTENT);
    let get_request = Request::builder().uri(&format!("/api/v1/packages/{}", test_package.name)).body(Body::empty())?;
    let response = app.oneshot(get_request).await?; assert_eq!(response.status(), StatusCode::NOT_FOUND);
    Ok(())
}
...
async fn create_test_app() -> Result<Router> {
    ...
    let app_state = AppState { package_service: package_service.clone(), deployment_service: deployment_service.clone(), crm_manager: crm_manager.clone() };
    let app = axum::Router::new()
        .route("/health", axum::routing::get(health_handler))
        .route("/api/v1/packages", axum::routing::post(handlers::create_package))
        .route("/api/v1/packages", axum::routing::get(handlers::list_packages))
        .route("/api/v1/packages/{name}", axum::routing::get(handlers::get_package))
        .route("/api/v1/packages/{name}", axum::routing::delete(handlers::delete_package))
        .route("/api/v1/clusters", axum::routing::get(handlers::list_clusters))
        .route("/api/v1/deployments", axum::routing::get(handlers::list_deployments))
